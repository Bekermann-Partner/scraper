"""
This module defines the `TagesschauSpider` class, which is a Scrapy spider used to
crawl news articles from the Tagesschau website (https://www.tagesschau.de/).
...
Usage:
    ...
    `scrapy crawl tagesschau_spider`

Attributes:
    None: This module does not define any module-level variables.

Example:
    - Start the spider: `scrapy crawl tagesschau_spider`
    - It will fetch news articles from the Tagesschau website.
"""
...
class TagesschauSpider(scrapy.Spider):
    """Spider to crawl and scrape news articles from Tagesschau's news website.

    The `TagesschauSpider` follows links from the main page and article pages. It
    ...
    custom_settings = {
        'DOWNLOAD_DELAY': 0.1  # 100ms delay between requests
    }
    ...
    def parse_article(self, response):
        ...
        if '/multimedia/' not in article_url:
            # Extract date from the paragraph with class "metatextline"
            article_date_str = response.css('p.metatextline::text').re_first(r'Stand: (\d{2}\.\d{2}\.\d{4})')
            ...
            if article_date_str:
                ...
                if article_date >= self.date_threshold:
                    ...
                    article_text = response.css('[class*="textabsatz"]::text').getall()
