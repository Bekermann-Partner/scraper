...
class BilderSpider(scrapy.Spider):
    """Spider to crawl and scrape news articles from Bild's news website.
    ...
    def parse_article(self, response):
        ...
        for link in links:
            ...
            if absolute_url not in self.visited_links and 'marktplatz.' not in absolute_url and 'sportdaten.' not in absolute_url:
                self.link_queue.put(absolute_url)

        # Only process valid article URLs
        if '/video/' not in article_url:
            # Extract the article's date
            article_date_str = response.css('time.datetime--article.datetime::attr(datetime)').get()
            ...
            if article_date_str:
                ...
                if article_date >= self.date_threshold:
                    ...
                    article_text = response.xpath('//div[contains(@class, "article-body")]//text()').getall()
