"""
This module defines the `StzSpider` class, which is a Scrapy spider used to
crawl news articles from the Stuttgarter Zeitung website (https://www.stuttgarter-zeitung.de/).
...
Usage:
    ...
    `scrapy crawl stz_spider`

Attributes:
    None: This module does not define any module-level variables.

Example:
    - Start the spider: `scrapy crawl stz_spider`
    - It will fetch news articles from the Stuttgarter Zeitung website.
"""
...
class StzSpider(scrapy.Spider):
    """Spider to crawl and scrape news articles from Stuttgarter Zeitung's news website.

    The `StzSpider` follows links from the main page and article pages. It
    ...
    """
    name = 'stz_spider'
    allowed_domains = ['stuttgarter-zeitung.de']
    start_urls = ['https://www.stuttgarter-zeitung.de/']
    ...
    def parse_article(self, response):
        ...
        for link in links:
            ...
            if (absolute_url not in self.visited_links and '/mediathek' not in absolute_url and 'stellenmarkt.' not in absolute_url
            and '/raetsel' not in absolute_url and '/podcasts' not in absolute_url):
                self.link_queue.put(absolute_url)

        # Only process valid article URLs
        if article_url:
            # Extract date from the paragraph with class "metatextline"
            article_date_str = response.css('span[itemprop="datePublished"]::text').re_first(r'\d{2}\.\d{2}\.\d{4}')
