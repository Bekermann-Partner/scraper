"""
This module defines the `SternSpider` class, which is a Scrapy spider used to
crawl news articles from the stern website (https://www.stern.de/).
...
Usage:
    ...
    `scrapy crawl stern_spider`

Attributes:
    None: This module does not define any module-level variables.

Example:
    - Start the spider: `scrapy crawl stern_spider`
    - It will fetch news articles from the stern website.
"""
...
class SternSpider(scrapy.Spider):
    """Spider to crawl and scrape news articles from Stern's news website.

    The `SternSpider` follows links from the main page and article pages. It
    ...
    def parse_article(self, response):
        ...
        for link in links:
            ...
            if (absolute_url not in self.visited_links and 'podcast/' not in absolute_url and '/video/' not in absolute_url
            and '/vergleich/' not in absolute_url and 'spiele.' not in absolute_url):
        ...
        if article_url:
            ...
            if article_date_str:
                ...
                if article_date >= self.date_threshold:
                    ...
                    article_text = response.css('div.text-element.u-richtext p::text').getall()
