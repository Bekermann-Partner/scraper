"""
This module defines the `FocusSpider` class, which is a Scrapy spider used to
crawl news articles from the Focus website (https://www.focus.de/).
...
Usage:
    ...
    `scrapy crawl focus_spider`

Attributes:
    None: This module does not define any module-level variables.

Example:
    - Start the spider: `scrapy crawl focus_spider`
    - It will fetch news articles from the Focus website.
"""
...
class FocusSpider(scrapy.Spider):
    """Spider to crawl and scrape news articles from Focus's news website.

    The `FocusSpider` follows links from the main page and article pages. It
    ...
    def parse_article(self, response):
        ...
        if '/videos/' not in article_url:
            # Extract the article's date
            article_date_str = response.css('div.displayDate::text').re_first(r'(\d{2}\.\d{2}\.\d{4})')
            ...
            if article_date_str:
                ...
                if article_date >= self.date_threshold:
                    ...
                    article_text = response.xpath(
                        '//div[contains(@class, "textBlock")]//text()'
                    ).getall()
