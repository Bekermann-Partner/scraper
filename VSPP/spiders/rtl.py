...
crawl news articles from the rlt website (https://www.rtl.de/).
...
Usage:
    ...
    `scrapy crawl rtl_spider`

Attributes:
    None: This module does not define any module-level variables.

Example:
    - Start the spider: `scrapy crawl rtl_spider`
...
class RtlSpider(scrapy.Spider):
    """Spider to crawl and scrape news articles from rtl's news website.

    The `RtlSpider` follows links from the main page and article pages. It
        ...
        if '/video/' not in article_url:
            ...
            if article_date_str:
                ...
                if article_date >= self.date_threshold:
                    ...
                    article_text = response.css('p.Typography_text-default__wldUd.Typography_align-left__koBUM.AnnotatedMarkup_paragraph__IUT9l::text').getall()
