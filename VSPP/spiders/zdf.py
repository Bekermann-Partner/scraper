...
class ZdfSpider(scrapy.Spider):
    """Spider to crawl and scrape news articles from ZDF's news website.
    ...
    def parse_article(self, response):
        ...
        if article_url.startswith('https://www.zdf.de/nachrichten/') and '/video/' not in article_url:
            # Extract the article's date
            article_date_str = response.css('span.dz5q5zg.m211382 time::attr(datetime)').get()
            ...
            if article_date_str:
                ...
                if article_date >= self.date_threshold:
                    ...
                    article_text = response.xpath(
                        '//div[contains(@class, "r1nj4qn5")]//text()'
                    ).getall()
