...
class SzSpider(scrapy.Spider):
    ...
    def parse_article(self, response):
        ...
        if article_url:
            ...
            article_date_str = response.css('time.css-b0z14z::attr(datetime)').get()

            if article_date_str:
                # Convert the date string to a datetime object
                article_date = datetime.strptime(article_date_str, "%Y-%m-%d %H:%M:%S").replace(tzinfo=timezone.utc)
                # Check if the article's date is after the threshold
                if article_date >= self.date_threshold:
                    # Extract article content
                    article_text = response.css('p.css-1yh3dil::text').getall()
