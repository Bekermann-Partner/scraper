...
class NtvSpider(scrapy.Spider):
    """Spider to crawl and scrape news articles from ntv's news website.
    ...
    def parse_article(self, response):
        ...
        if '/mediathek/' not in article_url:
            # Extract date from the paragraph with class "metatextline"
            article_date_str = response.css('span.article__date').re_first(r'\d{2}\.\d{2}\.\d{4}')
            ...
            if article_date_str:
                ...
                if article_date >= self.date_threshold:
                    ...
                    article_text = response.css('div.article__text p::text').getall()
