...
crawl news articles from the NDR website (https://www.ndr.de/).
...
Usage:
    ...
    `scrapy crawl ndr_spider`

Attributes:
    None: This module does not define any module-level variables.

Example:
    - Start the spider: `scrapy crawl ndr_spider`
...
class NdrSpider(scrapy.Spider):
    """Spider to crawl and scrape news articles from NDR's news website.

    The `NdrSpider` follows links from the main page and article pages. It
    ...
    start_urls = ['https://www.ndr.de/nachrichten']
    ...
    def parse_article(self, response):
        ...
        article_date_str = response.css('div.lastchanged::text').re_first(r'Stand: (\d{2}\.\d{2}\.\d{4})')

        if '/fernsehen/' not in article_url and '/podcasts/' not in article_url:
            if article_date_str:
                ...
                if article_date >= self.date_threshold:
                    ...
                    article_text = response.xpath(
                        '//p[not(ancestor::div[contains(@class, "teasertext")])]/text()'
                    ).getall()
