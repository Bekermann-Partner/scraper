"""
This module defines the `NiusSpider` class, which is a Scrapy spider used to
crawl news articles from the nius website (https://www.nius.de/).
...
Usage:
    ...
    `scrapy crawl nius_spider`

Attributes:
    None: This module does not define any module-level variables.

Example:
    - Start the spider: `scrapy crawl nius_spider`
    - It will fetch news articles from the nius website.
"""
...
class NiusSpider(scrapy.Spider):
    """Spider to crawl and scrape news articles from nius's news website.

    The `NiusSpider` follows links from the main page and article pages. It
    ...
    """
    name = 'nius_spider'
    allowed_domains = ['nius.de']
    start_urls = ['https://www.nius.de/all-news']
    # set user_agent via setting since website blocked access without it
    custom_settings = {
    'DEPTH_LIMIT': 0,
    ...
    def parse_article(self, response):
        ...
        for link in links:
            ...
            if (absolute_url not in self.visited_links and '/shows' not in absolute_url and '/clips' not in absolute_url
            and 'shop.' not in absolute_url and '/app' not in absolute_url):
                self.link_queue.put(absolute_url)

        # Only process valid article URLs
        if article_url:
            # Extract date from the paragraph with class "metatextline"
            article_date_str = response.css('div.article-aside_ArticleAside__date__aCCds::text').re_first(r'\d{2}\.\d{2}\.\d{4}')
            ...
            if article_date_str:
                ...
                if article_date >= self.date_threshold:
                    ...
                    article_text = response.css('div.article-main_ArticleMain__body__item__NmRTO p::text, div.article-main_ArticleMain__body__item__NmRTO strong::text').getall()
