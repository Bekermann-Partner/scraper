...
crawl news articles from the welt website (https://www.welt.de/).
...
Usage:
    ...
    `scrapy crawl welt_spider`

Attributes:
    None: This module does not define any module-level variables.

Example:
    - Start the spider: `scrapy crawl welt_spider`
    - It will fetch news articles from the welt website.
"""
...
class WeltSpider(scrapy.Spider):
    """Spider to crawl and scrape news articles from welt's news website.

    The `WeltSpider` follows links from the main page and article pages. It
    ...
    custom_settings = {
        'DOWNLOAD_DELAY': 0.2  # 200ms delay between requests
    }
    ...
    def parse_article(self, response):
        ...
        for link in links:
            ...
            if absolute_url not in self.visited_links and '/mediathek/' not in absolute_url and '/sportdaten/' not in absolute_url:
                self.link_queue.put(absolute_url)

        # Only process valid article URLs
        if article_url:
            # Extract date from the paragraph with class "metatextline"
            article_date_str = response.css('span.c-article-header__date::text').re_first(r'\d{2}\.\d{2}\.\d{4}')
            article_time_str = response.css('span.c-article-header__date::text').re_first(r'\d{2}:\d{2}')

            if article_date_str or article_time_str:
                ...
                if article_date >= self.date_threshold:
                    ...
                    article_text = response.xpath('//div[contains(@class, "c-rich-text-renderer")]//p/text()').getall()
