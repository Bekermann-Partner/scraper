"""
This module defines the `MdrSpider` class, which is a Scrapy spider used to
crawl news articles from the br website (https://www.mdr.de/).
...
Usage:
    ...
    `scrapy crawl mdr_spider`

Attributes:
    None: This module does not define any module-level variables.

Example:
    - Start the spider: `scrapy crawl mdr_spider`
    - It will fetch news articles from the mdr website.
"""
...
class MdrSpider(scrapy.Spider):
    """Spider to crawl and scrape news articles from mdr's news website.

    The `MdrSpider` follows links from the main page and article pages. It
    ...
    def parse_article(self, response):
        ...
        for link in links:
            ...
            if (absolute_url not in self.visited_links and '/video/' not in absolute_url and '/tv/' not in absolute_url
            and '/barrierefreiheit/' not in absolute_url and '/radio/' not in absolute_url and '/podcasts/' not in absolute_url):
                self.link_queue.put(absolute_url)

        # Only process valid article URLs
        if article_url:
            # Extract date from the paragraph with class "metatextline"
            article_date_str = response.css('p.webtime span::text').getall()

            if article_date_str:
                # Join the parts of the date string
                article_date_str = ''.join(article_date_str).strip()
                
                # Convert the date string to a datetime object
                article_date = datetime.strptime(article_date_str, "%d. %B %Y, %H:%M Uhr").replace(tzinfo=timezone.utc)
                # Check if the article's date is after the threshold
                if article_date >= self.date_threshold:
                    # Extract article content
                    article_text = response.css('p.text::text').getall()
