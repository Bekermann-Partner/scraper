...
class ZeitSpider(scrapy.Spider):
    """Spider to crawl and scrape news articles from zeit's news website.
    ...
    def parse_article(self, response):
        ...
        for link in links:
            ...
            if (absolute_url not in self.visited_links and '/zeit-magazin/' not in absolute_url and '/podcasts/' not in absolute_url and '/video/' not in absolute_url
            and 'shop.' not in absolute_url and 'jobs.' not in absolute_url and 'meine.' not in absolute_url and 'premium.' not in absolute_url and 'profile.' not in absolute_url
            and '/autoren/' not in absolute_url):
                self.link_queue.put(absolute_url)

        # Only process valid article URLs
        if article_url:
            # Extract date from the paragraph with class "metatextline"
            article_date_str = response.css('time.metadata__date::attr(datetime)').get()
            ...
            if article_date_str:
                ...
                if article_date >= self.date_threshold:
                    ...
                    article_text = response.css('p.paragraph.article__item::text').getall()
