"""
This module defines the `TazSpider` class, which is a Scrapy spider used to
crawl news articles from the faz website (https://taz.de/).
...
Usage:
    ...
    `scrapy crawl taz_spider`

Attributes:
    None: This module does not define any module-level variables.

Example:
    - Start the spider: `scrapy crawl taz_spider`
    - It will fetch news articles from the taz website.
"""
...
class TazSpider(scrapy.Spider):
    """Spider to crawl and scrape news articles from frankfurter Rundschau's news website.

    The `TazSpider` follows links from the main page and article pages. It
    ...
    def parse_article(self, response):
        ...
        for link in links:
            ...
            if (absolute_url not in self.visited_links and 'epaper.' not in absolute_url and 'shop.' not in absolute_url
            and '/verlag/' not in absolute_url and '/Podcast-Bundestalk/' not in absolute_url):
                self.link_queue.put(absolute_url)

        # Only process valid article URLs
        if article_url:
            # Extract date from the paragraph with class "metatextline"
            article_date_str = response.css('time::text').re_first(r'\d{2}\.\d{2}\.\d{4}')
            ...
            if article_date_str:
                ...
                if article_date >= self.date_threshold:
                    ...
                    article_text = response.xpath('//div[@class="columns is-multiline"]//p[contains(@class, "paragraph")]/text()').getall()
