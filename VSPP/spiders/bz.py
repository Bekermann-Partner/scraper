"""
This module defines the `BzSpider` class, which is a Scrapy spider used to
crawl news articles from the Berliner Zeitung website (https://www.berliner-zeitung.de/).
...
Usage:
    ...
    `scrapy crawl bz_spider`

Attributes:
    None: This module does not define any module-level variables.

Example:
    - Start the spider: `scrapy crawl bz_spider`
    - It will fetch news articles from the berliner Zeitung website.
"""
...
class BzSpider(scrapy.Spider):
    """Spider to crawl and scrape news articles from berliner Zeitung's news website.

    The `BzSpider` follows links from the main page and article pages. It
    ...
    # Since it turned out that they don't like it when you don't set a download delay for the spider
    custom_settings = {
        'DOWNLOAD_DELAY': 0.4  # 400ms delay between requests
    }
    ...
    def parse_article(self, response):
        ...
        for link in links:
            ...
            if (absolute_url not in self.visited_links and '/open-source/' not in absolute_url and '/ticketshop' not in absolute_url
            and 'epaper.' not in absolute_url and '/podcast' not in absolute_url):
                self.link_queue.put(absolute_url)

        # Only process valid article URLs
        if article_url:
            # Extract date from the paragraph with class "metatextline"
            article_date_str = response.css('time > span::text').re_first(r'\d{2}\.\d{2}\.\d{4}')
            ...
            if article_date_str:
                ...
                if article_date >= self.date_threshold:
                    ...
                    article_text = response.css('p.article_paragraph__hXYKJ::text').getall()
